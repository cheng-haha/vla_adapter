...
multi_queries_num=75
multi_query_norm_type=layernorm
action_norm=layernorm
use_residual=False
num_residual_heads=3
split_inputs=False
MODE=${RUN_MODE}_prop_inner${expand_inner_ratio}_proj_type_${proj_type}_ffn_type_${ffn_type}_mlp_${mlp_type}_decoder_num_blocks_${decoder_num_blocks}
#========== !NOTE! ==========#
...
#========== run ==========#
WANDB_CONSOLE=off WANDB_MODE=offline torchrun --standalone --nnodes 1 --nproc-per-node 4 vla-scripts/finetune_residual.py \
  --vla_path "$vla_path" \
