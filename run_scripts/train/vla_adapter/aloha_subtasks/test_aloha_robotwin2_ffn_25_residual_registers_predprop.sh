...
proj_type=onlynorm
ffn_type=swiglu
expand_actiondim_ratio=1
linear_drop_ratio=0.0
multi_queries_num=25
multi_query_norm_type=dyt
action_norm=dyt
use_residual=False
num_residual_heads=4
split_inputs=False
use_velocity_loss=False
use_each_norm=False
MODE=${RUN_MODE}_q${multi_queries_num}_r${num_residual_heads}_d${expand_actiondim_ratio}_n${multi_query_norm_type}_proj_type_${proj_type}_ffn_type_${ffn_type}_mlp_${mlp_type}_decoder_num_blocks_${decoder_num_blocks}
#========== !NOTE! ==========#
...
WANDB_CONSOLE=off WANDB_MODE=offline torchrun --standalone --nnodes 1 --nproc-per-node 4 vla-scripts/finetune_preprop.py \
  ...
  --use_velocity_loss "$use_velocity_loss"\
  --use_each_norm "$use_each_norm"
