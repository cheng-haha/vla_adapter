...
ffn_type=gelu
expand_actiondim_ratio=0.25
linear_drop_ratio=0.0
multi_queries_num=1
multi_query_norm_type=layernorm
action_norm=layernorm
use_query_action_head=True
MODE=${RUN_MODE}_exffn${expand_actiondim_ratio}_proj_type_${proj_type}_ffn_type_${ffn_type}_mlp_${mlp_type}_decoder_num_blocks_${decoder_num_blocks}
#========== !NOTE! ==========#
...
WANDB_CONSOLE=off WANDB_MODE=offline torchrun --standalone --nnodes 1 --nproc-per-node 4 vla-scripts/finetune.py \
  ...
  --use_query_action_head "$use_query_action_head"