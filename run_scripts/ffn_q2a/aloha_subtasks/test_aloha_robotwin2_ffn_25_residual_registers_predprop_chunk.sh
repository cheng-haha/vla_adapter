...
#========== !NOTE! ==========#
RUN_MODE=simvla_phone_stand_25
...
future_proprio_chun_size=24
multi_queries_num=25
multi_queries_num=$((multi_queries_num + future_proprio_chun_size))
echo "multi_queries_num: $multi_queries_num"
multi_query_norm_type=layernorm
action_norm=layernorm
use_residual=False
num_residual_heads=1
split_inputs=False
use_chunk_wise_proprio=True
MODE=${RUN_MODE}_q${multi_queries_num}_inner${expand_inner_ratio}_proj_type_${proj_type}_ffn_type_${ffn_type}_mlp_${mlp_type}_decoder_num_blocks_${decoder_num_blocks}
#========== !NOTE! ==========#
...
dataset_name=place_phone_stand_aloha_agilex_50
...
#========== run ==========#
WANDB_CONSOLE=off WANDB_MODE=offline torchrun --standalone --nnodes 1 --nproc-per-node 4 vla-scripts/finetune_preprop.py \
  --vla_path "$vla_path" \
  ...
  --use_chunk_wise_proprio "$use_chunk_wise_proprio"
