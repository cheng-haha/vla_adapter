...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_deep_recursion:
        # Separate optimizers for VLM and policy
        vlm_params = [p for p in vla.parameters() if p.requires_grad]
        if cfg.use_proprio:
            vlm_params += [p for p in proprio_projector.parameters() if p.requires_grad]
        ...
        original_lr_policy = optimizer_policy.param_groups[0]["lr"]

        scheduler_vlm = MultiStepLR(optimizer_vlm, milestones=[cfg.num_steps_before_decay], gamma=0.1)
        scheduler_policy = MultiStepLR(optimizer_policy, milestones=[cfg.num_steps_before_decay], gamma=0.1)
    else:
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            if cfg.use_deep_recursion:
                ...
                # VLA forward pass to get x_query
                with torch.autocast("cuda", dtype=torch.bfloat16):
                    output: CausalLMOutputWithPast = vla(
                        input_ids=batch["input_ids"].to(device_id),
                        attention_mask=batch["attention_mask"].to(device_id),
                        ...
                        proprio_as_queries=cfg.proprio_as_queries if cfg.use_proprio else False,
                        use_film=cfg.use_film,
                    )
                ...
                for item in output.hidden_states[0:]:
                    text_hidden_states = item[:, NUM_PATCHES:-1]
                    batch_size = batch["input_ids"].shape[0]
                    current_action_mask = get_current_action_mask(batch["labels"][:,1:].to(device_id))
                    next_actions_mask = get_next_actions_mask(batch["labels"][:,1:].to(device_id))
                    actions_hidden_states = text_hidden_states[current_action_mask | next_actions_mask].reshape(batch_size, 1, NUM_TOKENS, -1).to(torch.bfloat16)
                    task_latten_states = item[:, :NUM_PATCHES].reshape(batch_size, 1, NUM_PATCHES, -1)
                    all_hidden_states = torch.cat((task_latten_states, actions_hidden_states), 2)
                    multi_layer_hidden_states.append(all_hidden_states)
                ...
                loss_vlm = F.l1_loss(predicted_actions, ground_truth_actions)
                ...
                with torch.no_grad():
                    ...
                    next_actions_l1_loss = F.l1_loss(ground_truth_next_actions, predicted_next_actions)
                    print(
                        f"VLM Losses - Current Action: {curr_action_l1_loss.item():.6f}, "
                        f"Next Actions: {next_actions_l1_loss.item():.6f}"
                    )
                ...
                # Since deep recursion does not use gradient accumulation, each batch is a step.
                gradient_step_idx = batch_idx
            ...
            if cfg.lr_warmup_steps > 0:
                step_for_lr = batch_idx if cfg.use_deep_recursion else (batch_idx // cfg.grad_accumulation_steps)
                lr_progress = min((step_for_lr + 1) / cfg.lr_warmup_steps, 1.0)  # Cap at 1.0
                if cfg.use_deep_recursion:
                    current_lr_vlm = original_lr_vlm * (0.1 + 0.9 * lr_progress)
                    current_lr_policy = original_lr_policy * (0.1 + 0.9 * lr_progress)
                    for param_group in optimizer_vlm.param_groups:
                        param_group["lr"] = current_lr_vlm
                    for param_group in optimizer_policy.param_groups:
                        param_group["lr"] = current_lr_policy
                else:
            ...
            if distributed_state.is_main_process and gradient_step_idx % cfg.wandb_log_freq == 0:
                ...
                wandb.log(
                    {
                        "VLA Train/Learning Rate": scheduler_vlm.get_last_lr()[0] if cfg.use_deep_recursion else scheduler.get_last_lr()[0],
                    },
                    step=log_step,
                )
                print(f"Step {log_step} - Learning Rate: {scheduler_vlm.get_last_lr()[0] if cfg.use_deep_recursion else scheduler.get_last_lr()[0]:.10e}")

            # Optimizer and LR scheduler step
            if not cfg.use_deep_recursion and (batch_idx + 1) % cfg.grad_accumulation_steps == 0:
                optimizer.step()
                scheduler.step()
                optimizer.zero_grad()
                progress.update()
