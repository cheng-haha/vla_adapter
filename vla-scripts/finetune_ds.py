...
class FinetuneConfig:
    ...
    # Deep Supervision
    deep_supervise: bool = False                       # If True, applies deep supervision to intermediate layers of the action head
    deep_supervise_ensemble: bool = False              # If True, ensembles predictions from different layers for deep supervision
...
def run_forward_pass(
    ...
    # Compute metrics for continuous action representations (L1 regression)
        ...
        if cfg.deep_supervise_ensemble:
            # Main loss is on the ensembled prediction
            main_loss = torch.nn.L1Loss()(predicted_actions, ground_truth_actions_for_loss)
            loss = main_loss
            metrics["ensembled_action_l1_loss"] = main_loss.item()

            # Log individual layer losses for analysis
            if coarse_actions is not None and isinstance(coarse_actions, list):
                coarse_loss_low = torch.nn.L1Loss()(coarse_actions[0], ground_truth_actions_for_loss)
                coarse_loss_mid = torch.nn.L1Loss()(coarse_actions[1], ground_truth_actions_for_loss)
                coarse_loss_high = torch.nn.L1Loss()(coarse_actions[2], ground_truth_actions_for_loss)
                metrics["coarse_action_l1_loss_low"] = coarse_loss_low.item()
                metrics["coarse_action_l1_loss_mid"] = coarse_loss_mid.item()
                metrics["coarse_action_l1_loss_high"] = coarse_loss_high.item()
                print(f"Deep Supervision Losses - Low: {coarse_loss_low.item():.6f}, Mid: {coarse_loss_mid.item():.6f}, High: {coarse_loss_high.item():.6f}")

        elif cfg.deep_supervise:
            # Loss is the sum of losses from all supervised layers
            if coarse_actions is not None and isinstance(coarse_actions, list):
                coarse_loss_low = torch.nn.L1Loss()(coarse_actions[0], ground_truth_actions_for_loss)
                coarse_loss_mid = torch.nn.L1Loss()(coarse_actions[1], ground_truth_actions_for_loss)
                coarse_loss_high = torch.nn.L1Loss()(coarse_actions[2], ground_truth_actions_for_loss)
                loss = coarse_loss_low + coarse_loss_mid + coarse_loss_high
                metrics["coarse_action_l1_loss_low"] = coarse_loss_low.item()
                metrics["coarse_action_l1_loss_mid"] = coarse_loss_mid.item()
                metrics["coarse_action_l1_loss_high"] = coarse_loss_high.item()
                print(f"Deep Supervision Losses - Low: {coarse_loss_low.item():.6f}, Mid: {coarse_loss_mid.item():.6f}, High: {coarse_loss_high.item():.6f}")
            else:
                loss = torch.tensor(0.0, device=device_id, requires_grad=True) # Should not happen
