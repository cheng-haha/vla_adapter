...
class FinetuneConfig:
    ...
    phase: str = "Training"
    # fmt: on
    use_diversity_loss: bool = False                   # Whether to use diversity loss
    diversity_loss_weight: float = 0.1                 # Weight for the diversity loss
...
def calculate_diversity_loss(token_features: torch.Tensor) -> torch.Tensor:
    if token_features.shape[1] <= 1:  # N <= 1
        return torch.tensor(0.0, device=token_features.device)

    B, N, D = token_features.shape

    normalized_features = F.normalize(token_features, p=2, dim=-1)
    cosine_sim = torch.bmm(normalized_features, normalized_features.transpose(1, 2))  # (B, N, N)

    # Create a mask for the upper triangle, excluding the diagonal
    mask = torch.triu(torch.ones(N, N, device=cosine_sim.device), diagonal=1).bool()

    # Apply the mask to each slice in the batch and flatten the result
    upper_triangle_sims = cosine_sim.masked_select(mask)

    # Mean of similarities in the upper triangle across the whole batch
    return upper_triangle_sims.mean()
...
def run_forward_pass(
    ...
    # Compute metrics for continuous action representations (L1 regression)
        ...
        diversity_loss = torch.tensor(0.0, device=multi_layer_hidden_states.device)
        if action_head.module.training and cfg.use_diversity_loss:
            action_tokens_multi_layer = multi_layer_hidden_states[:, :, num_patches:, :]

            if cfg.only_simple_action_head and not cfg.deep_supervise and not cfg.deep_supervise_ensemble:
                if cfg.ensemble_hidden_state:
                    actions_hidden_states = action_tokens_multi_layer.mean(dim=1)
                else:
                    actions_hidden_states = action_tokens_multi_layer[:, -1, :, :]
                diversity_loss = calculate_diversity_loss(actions_hidden_states)

            elif cfg.deep_supervise or cfg.deep_supervise_ensemble:
                hidden_low = action_tokens_multi_layer[:, 4, :, :]
                hidden_mid = action_tokens_multi_layer[:, 16, :, :]
                hidden_high = action_tokens_multi_layer[:, -1, :, :]
                loss_low = calculate_diversity_loss(hidden_low)
                loss_mid = calculate_diversity_loss(hidden_mid)
                loss_high = calculate_diversity_loss(hidden_high)
                diversity_loss = (loss_low + loss_mid + loss_high) / 3.0
        ...
        if cfg.use_diversity_loss and action_head.module.training:
            loss += cfg.diversity_loss_weight * diversity_loss
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_diversity_loss:
        recent_metrics["diversity_loss"] = deque(maxlen=cfg.grad_accumulation_steps)
