...
class GenerateConfig:
    ...
    vla_server_url: str = "localhost:8000"           # URL of the VLA server
...
def validate_config(cfg: GenerateConfig) -> None:
    ...
    if not cfg.use_vla_server:
        assert cfg.pretrained_checkpoint, "A `pretrained_checkpoint` must be provided when `use_vla_server` is False."
...
def check_unnorm_key(cfg: GenerateConfig, model=None) -> None:
    """Check that the model contains the action un-normalization key."""
    # Initialize unnorm_key
    unnorm_key = cfg.task_suite_name

    if model is None:
        # In client mode, we can't check the model's norm_stats.
        # We assume the standard key for libero suites.
...
def run_episode(
    ...
    initial_state=None,
    log_file=None,
    client: Optional[MsgPackHttpClientPolicy] = None,
):
        ...
        while t < max_steps + cfg.num_steps_wait:
            ...
            if len(action_queue) == 0:
                if cfg.use_vla_server:
                    assert client is not None, "Client must be provided when using VLA server."
                    # Prepare observation for the server
                    server_observation = {
                    ...
                    # Query the VLA server for actions.
                    actions = np.array(client.infer(server_observation)["actions"])
                else:
